...
func (db *DB) Seed() error {
	...
	for _, number := range data {
		if _, err := insertPhone(db.conn, number); err != nil {
			return err
		}
	}
	return nil
...
func (db *DB) AllPhones() ([]Phone, error) {
	rows, err := db.conn.Query("SELECT id, value FROM phone_numbers")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) FindPhone(number string) (*Phone, error) {
	...
	err := db.conn.QueryRow(
		"SELECT * FROM phone_numbers WHERE value=$1",
		number,
	).Scan(&p.ID, &p.Number)
	if err != nil {
...
func (db *DB) UpdatePhone(p *Phone) error {
	...
	_, err := db.conn.Exec(statement, p.ID, p.Number)
	return err
}
